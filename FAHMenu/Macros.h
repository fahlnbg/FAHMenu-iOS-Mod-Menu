...
uint64_t getRealOffset(uint64_t offset){
	return KittyMemory::getAbsoluteAddress([UIPatch getFrameworkName], offset);
}

// Patching a offset without switch.
void patchOffset(uint64_t offset, std::string hexBytes) {
	MemoryPatch patch = MemoryPatch::createWithHex([UIPatch getFrameworkName], offset, hexBytes);
	if(!patch.isValid()){
